...
/**
 * 通过单例模式的学习告诉我们：
 * a、单例模式理解起来简单，但是具体实现起来还是有一定的难度。
 * b、synchronized关键字锁定的是对象，在用的时候，一定要在恰当的地方使用（注意需要使用锁的对象和过程，可能有的时候并不是整个对象及整个过程都需要锁）。
...
public class SingletonOther {
     ...
     * 注意这里是static
     */
    private static SingletonOther singleton = null;
    ...
    public void updateProperties(){
        SingletonOther singletonOther = new SingletonOther();
        properties = singletonOther.getProperties();
    }
